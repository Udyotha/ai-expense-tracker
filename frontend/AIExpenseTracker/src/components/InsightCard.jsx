...
const MonthlySummaryView = ({ c, currency = 'USD' }) => {
            ...
            {typeof c.estimatedMonthlySavings === 'number' && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <Stat
                        ...
                        value={`${symbol}${Number(c.estimatedMonthlySavings).toLocaleString()}/mo`}
                        accent="emerald"
                    />
                    <Stat
...
const SavingsTipsView = ({ c, currency = 'USD' }) => {
        ...
        <div className="space-y-5">
            ...
            {totalSavings > 0 && (
                <div className="grid grid-cols-2 gap-3">
                    <Stat label="Total potential" value={`${symbol}${totalSavings.toFixed(0)}/mo`} accent="emerald" />
                            ...
                            <div className="flex items-center justify-between mb-3">
                                ...
                                {savings > 0 && (
                                    <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-full">
                                        <TrendingUp size={11} />~{symbol}{savings}/mo
                                    </span>
                                )}
                            </div>
...
const headerChip = (insight, currency = 'USD') => {
    ...
    if (insight.insight_type === 'savings_tips') {
        ...
        if (total > 0) {
            const symbol = getCurrencySymbol(currency);
            return <span className="text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full bg-emerald-50 text-emerald-700">~{symbol}{total.toFixed(0)}/mo</span>;
        }
    }
    return null;
};

const InsightCard = ({ insight, defaultExpanded = false, currency = 'USD' }) => {
        ...
        <div className="bg-white rounded-3xl border border-slate-100 overflow-hidden hover:border-slate-200 transition">
            ...
            {expanded && (
                <div className="px-5 pb-6 border-t border-slate-100 pt-5">
                    {insight.insight_type === 'monthly_summary' && <MonthlySummaryView c={c} currency={currency} />}
                    {insight.insight_type === 'budget_alert' && <BudgetAlertView c={c} />}
                    {insight.insight_type === 'savings_tips' && <SavingsTipsView c={c} currency={currency} />}
                </div>
            )}
        </div>
    );
