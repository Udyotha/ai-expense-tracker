...
const KpiCard = ({ label, value, delta, icon: Icon, accent = 'slate', className = '' }) => {
    const hasDelta = delta != null && Number.isFinite(delta);
    const positive = hasDelta && delta >= 0;
    const gradient = gradients[accent] || gradients.slate;

    return (
        <div className={`bg-white rounded-3xl border border-slate-100 p-5 flex items-center gap-4 ${className}`}>
            {Icon && (
